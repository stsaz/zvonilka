...
# zvonilka: cross-build on Linux for Android
...
# GRADLE_DIR=
# CPU=
IMAGE_NAME=zvonilka-android-builder
CONTAINER_NAME=zvonilka_android_build
ARGS=${@@Q}
...
EOF
	...
	podman create --attach --tty \
	 ...
	 --name $CONTAINER_NAME \
	 $IMAGE_NAME \
	 bash -c 'cd /src/zvonilka && source ./build_android.sh'
fi
...
mkdir -p ../phiola/alib3/_android-$CPU
make -j8 opus soxr \
 -C ../phiola/alib3/_android-$CPU \
 -f ../Makefile \
 -I .. \
 COMPILER=clang \
 SYS=android \
